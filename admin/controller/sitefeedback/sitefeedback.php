...
class ControllerSitefeedbacksitefeedback extends Controller
{
    private $error = array();

    public function index()
    {
        //$this->load->language('sitefeedback/sitefeedback');

		$this->load->language('seller/seller');

        $this->document->setTitle($this->language->get('heading_title_feed'));

        $this->load->model('sitefeedback/sitefeedback');
    ...
    public function approve()
    {
        ...
        if (isset($this->request->post['selected'])) {
            ...
            $sellers[] = $this->request->get['feedback_id'];
        }

        if ($this->validateApprove()) {
            //$approve_comment = $this->language->get('approve_ad_comment');
            //$this->model_seller_seller->addHistory($this->request->get['feedback_id'], $approve_comment);

			$this->model_sitefeedback_sitefeedback->addUserActivity($this->request->get['feedback_id'], 'feedback_status');
            ...
            $this->model_sitefeedback_sitefeedback->approve($this->request->get['feedback_id']);
            ...
            $this->response->redirect($this->url->link('sitefeedback/sitefeedback', 'token='.$this->session->data['token'].$url, 'SSL'));
        }
    ...
    public function disapprove()
    {
        ...
        if ($this->validatedisapprove()) {
			...
			if ($this->request->post['remarks'] == '') {            
				...
				$this->model_sitefeedback_sitefeedback->addUserActivity($this->request->post['feedback_id'], 'ad_rejected');

				$this->model_sitefeedback_sitefeedback->disapprove($this->request->post);
    ...
    protected function getList()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('sitefeedback/sitefeedback', 'token='.$this->session->data['token'].$url, 'SSL'),
        );

		//$data['request'] = $this->request->get['filter_ad_status'];

        $data['insert'] = $this->url->link('sitefeedback/sitefeedback/add', 'token='.$this->session->data['token'].$url, 'SSL');
        $data['delete'] = $this->url->link('sitefeedback/sitefeedback/delete', 'token='.$this->session->data['token'].$url, 'SSL');

        $data['sitefeedback'] = array();
...
//print_r($filter_data);die;
        $sitefeedback_total = $this->model_sitefeedback_sitefeedback->getTotalsitefeedback($filter_data);
		//print_r($sitefeedback_total);die;

        $results = $this->model_sitefeedback_sitefeedback->getsitefeedback($filter_data);
        ...
        foreach ($results as $result) {
            ...
            $data['sitefeedback'][] = array(
                ...
                'approve' => $this->url->link('sitefeedback/sitefeedback/approve', 'token='.$this->session->data['token'].'&feedback_id='.$result['feedback_id'].$url, 'SSL'),
                ...
                'disapprove' => $this->url->link('sitefeedback/sitefeedback/disapprove', 'token='.$this->session->data['token'].'&feedback_id='.$result['feedback_id'].$url, 'SSL'),
            );
        }

        $data['heading_title_feed'] = $this->language->get('heading_title_feed');
        ...
        $data['sort_date_added'] = $this->url->link('sitefeedback/sitefeedback', 'token='.$this->session->data['token'].'&sort=date_added'.$url, 'SSL');
        ...
        $pagination->total = $sitefeedback_total;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_limit_admin');
        $pagination->url = $this->url->link('sitefeedback/sitefeedback', 'token='.$this->session->data['token'].$url.'&page={page}', 'SSL');

        $data['pagination'] = $pagination->render();

        $data['results'] = sprintf($this->language->get('text_pagination'), ($sitefeedback_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($sitefeedback_total - $this->config->get('config_limit_admin'))) ? $sitefeedback_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $sitefeedback_total, ceil($sitefeedback_total / $this->config->get('config_limit_admin')));
        ...
        $this->response->setOutput($this->load->view('sitefeedback/site_feedback.tpl', $data));
    }
    ...
    {
        if (!$this->user->hasPermission('modify', 'sitefeedback/sitefeedback')) {
