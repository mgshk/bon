...
class ModelSelleradvertiseselleradvertise extends Model
{
    ...
    public function getselleradvertise($data = array())
    {
        $sql = "SELECT * FROM  ".DB_PREFIX."store_offers ";
		//print_r($data);
        $implode = array();
		...
		if (isset($data['filter_ad_status']) && $data['filter_ad_status'] !== null) {
            $implode[] = "status = '".$data['filter_ad_status']."'";
        }
        ...
        if (!empty($data['filter_date_added'])) {
            $implode[] = "DATE(date_modified) = DATE('".$this->db->escape($data['filter_date_added'])."')";
        }
	...
	public function addUserActivity($advertise_id, $action)
    {
		$sql = 'INSERT INTO '.DB_PREFIX."user_activity SET user_id = '".(int) $this->user->getId()."', activity_id = '".(int) $advertise_id."', key_name = '".$action."', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', date_added = NOW()";
		//echo $sql; die;
		$this->db->query($sql);
	}

    public function approve($advertise_id)
    {
        //$seller_info = $this->getseller($advertise_id);

        //if ($seller_info) {
            $this->db->query('UPDATE '.DB_PREFIX."store_offers SET status = 'approved' ,date_modified = NOW() WHERE advertise_id = '".(int) $advertise_id."'");
    ...
    public function disapprove($data = array())
    {
        //$seller_info = $this->getseller($seller_id);

        //if ($seller_info) {
            $this->db->query('UPDATE '.DB_PREFIX."store_offers SET status = 'rejected' , remarks = '".$data['remarks']."',date_modified = NOW() WHERE advertise_id = '".(int) $data['advertise_id']."'");
    ...
    public function getTotalselleradvertise($data = array())
    {
        $sql = 'SELECT COUNT(*) AS total FROM '.DB_PREFIX.'store_offers';
		//print_r($data);
        $implode = array();
    ...
    public function addHistory($advertise_id, $comment)
    {
