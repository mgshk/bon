...
class ModelSellerseller extends Model
{
    ...
    public function getsellers($data = array())
    {
        ...
        WHERE c.nickname != '' AND cgd.language_id = '".(int) $this->config->get('config_language_id')."'";
	...
	public function verify($seller_id)
    {
        $seller_info = $this->getseller($seller_id);

        if ($seller_info) {
            $this->db->query('UPDATE '.DB_PREFIX."customer SET seller_verified = '1' ,seller_date_added = NOW() WHERE customer_id = '".(int) $seller_id."'");
    ...
    public function disapprove($seller_id, $reason)
    {
        $seller_info = $this->getseller($seller_id);

        if ($seller_info) {
            $this->db->query('UPDATE '.DB_PREFIX."customer SET seller_approved = '0', seller_reject_reason = '".$reason."' WHERE customer_id = '".(int) $seller_id."'");
	...
	public function disverify($seller_id)
    {
        ...
        if ($seller_info) {
            $this->db->query('UPDATE '.DB_PREFIX."customer SET seller_verified = '0' WHERE customer_id = '".(int) $seller_id."'");            
        }
    }
