...
class ModelSitefeedbacksitefeedback extends Model
{
    

    

    public function getsitefeedback($data = array())
    {
        $sql = "SELECT * FROM  ".DB_PREFIX."site_feedback ";
		//print_r($data);
        $implode = array();
    ...
    {
		$this->db->query('INSERT INTO '.DB_PREFIX."user_activity SET user_id = '".(int) $this->user->getId()."', activity_id = '".(int) $advertise_id."', key_name = '".$action."', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', date_added = NOW()");
	}
    ...
    public function getTotalsitefeedback($data = array())
    {//echo "test"; die;
        $sql = 'SELECT COUNT(*) AS total FROM '.DB_PREFIX.'site_feedback';
		//print_r($data);
        $implode = array();
