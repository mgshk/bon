...
class Controllersellerprofilesellerprofile extends Controller
{
    ...
    public function profile()
    {
		...
		if($this->request->get['tin'] != '' && $this->request->get['pan'] == ''){
			$json['error'] = $this->language->get('error_tin');
		} elseif($this->request->get['tin'] == '' && $this->request->get['pan'] != '') {
			$json['error'] = $this->language->get('error_pan');
		} elseif($this->request->get['nickname'] != '' && (utf8_strlen(trim($this->request->get['nickname'])) > 200)) {
			$json['error'] = $this->language->get('error_nickname');
		} elseif($this->request->get['seller_description'] != '' && (utf8_strlen(trim($this->request->get['seller_description'])) > 2000)) {
			$json['error'] = $this->language->get('error_seller_description');
		} elseif($this->request->get['owner_name'] != '' && (utf8_strlen(trim($this->request->get['owner_name'])) > 200)) {
			$json['error'] = $this->language->get('error_owner_name');
		} elseif($this->request->get['store_email'] != '' && (utf8_strlen(trim($this->request->get['store_email'])) > 100)) {
			$json['error'] = $this->language->get('error_store_email');
		} elseif($this->request->get['facebook'] != '' && (utf8_strlen(trim($this->request->get['facebook'])) > 500)){
			$json['error'] = $this->language->get('error_facebook');
		} elseif($this->request->get['twitter'] != '' && (utf8_strlen(trim($this->request->get['twitter'])) > 500)) {
			$json['error'] = $this->language->get('error_twitter');
		} elseif($this->request->get['googleplus'] != '' && (utf8_strlen(trim($this->request->get['googleplus'])) > 500)) {
			$json['error'] = $this->language->get('error_googleplus');
		} elseif($this->request->get['website'] != '' && (utf8_strlen(trim($this->request->get['website'])) > 500)) {
			$json['error'] = $this->language->get('error_website');
		} elseif($this->request->get['instagram'] != '' && (utf8_strlen(trim($this->request->get['instagram'])) > 500)) {
			$json['error'] = $this->language->get('error_instagram');
		} elseif(isset($this->request->get['referred_by']) != '' && (utf8_strlen(trim(isset($this->request->get['referred_by']))) != 10)) {
			$json['error'] = $this->language->get('error_referred_by');
		} elseif(!empty($this->request->get['store_mobile_num'])) {
			$str_mob_filter = array_filter($this->request->get['store_mobile_num']);//print_r($str_mob_filter); die;
			foreach($str_mob_filter as $str_mb_num) {
				if(utf8_strlen(trim($str_mb_num)) != 10) {
		...
		if((!empty($this->request->get['store_ll_num'])) || (!empty($this->request->get['store_ll_code']))) {
			foreach($this->request->get['store_ll_num'] as $store_ll_num) {
				if($store_ll_num == '') {
					$json['error'] = $this->language->get('error_store_ll_num');
				}
			}
			foreach($this->request->get['store_ll_code'] as $store_ll_code) {//print_r($this->request->get['store_ll_code']); die;
				if($store_ll_code == '') {
					$json['error'] = $this->language->get('error_store_ll_num');
				}
			}
	...
	public function sellerAdvertisedraft()
    {
        ...
        foreach ($results as $result) {
            if (is_file(DIR_IMAGE.$result['offer_image'])) {
                $image = $this->model_tool_image->resize($result['offer_image'], 1320, 230);
            } else {
                $image = $this->model_tool_image->resize('no_image.png', 1320, 230);
            }

			if (is_file(DIR_IMAGE.$result['offer_image'])) {
				$image_tumb = $this->model_tool_image->resize($result['offer_image'], 1320, 230);
			} else {
				$image_tumb = $this->model_tool_image->resize('no_image.png', 1320, 230);
			}
	...
	public function move_live()
    {
		...
		//if(isset($this->request->post['loc']) && $this->request->post['loc'] == '') {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate_move_live()) {			
			if(($this->request->post['from_date'] != '') && ($this->request->post['end_date'] != '') && ($this->request->post['position'] != '')){
				...
				if($loc == '2' || $loc == '3' || $loc == '4' || $loc == '5') {
					$position_amount = $this->model_selleradvertise_advertise->getPostionAmount($from_date, $end_date, $loc, $ad_order);				
					if($position_amount > 0){
						$value = array();
						foreach($position_amount as $key => $position) {							
							if($key == 0) {
								$value[0] = $position['price'];
							}
							if($key == 1) {
								$value[1] = $position['price'];
							}							
						}
						if($value > 0 && !empty($value)) {
							if(count($value) == 1) {								
								if($amount < $value['0']) { 
									$this->model_selleradvertise_advertise->adToLive($this->request->post);
									$json['success'] = $this->language->get('success_to_live');	
								} else {
									$this->error['amount'] = $this->language->get('position_amount_max');
									$json['amount'] = $this->error['amount'];
								}
							} else {								
								$bt_amount = $value['0'] - $value['1'];
								if($bt_amount > 1) {
									...
									if(($amount > $value['1']) && ($amount <= $value['0'])) { 
										...
										//$this->error['position'] = $this->language->get('position_amount_min');
										$json['amount'] = "Enter Amount ".$value['0'];
									}
								}
							}
						} else { 
			...
			if($this->request->post['loc'] == '1') {
				...
				if($top_banner_date != ''){	
					...
					foreach($top_banner_date_arr as $key => $top_banner_date) {
						...
						$data['price'] = $default_home_top_advertise_amount = 5000 * ($key +1);
...
//print_r($this->request->post['loc']);
    ...
    {
		$json = array();
		$default_seller_advertise_amount = 500;
			...
			if(($this->request->post['from_date'] != '') && ($this->request->post['end_date'] != '') && ($this->request->post['position'] != '')){				
				...
				if($position_amount != ''){
					if($position_amount > 0){
						...
						$json['success'] = array('0' => $default_seller_advertise_amount);
					}
				}
			}
		}
