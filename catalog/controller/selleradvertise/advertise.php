...
class ControllerselleradvertiseAdvertise extends Controller
{
    ...
    {//print_r("test");die;
		...
		$this->document->addScript('crop/cropper.js');
		//$this->document->addScript('crop/main.js');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {			
			...
			if($this->request->post['image_crop'] != '') {
				...
				$angle = '0';
				//$image_crop = str_replace('data:image/png;base64,', '', $this->request->post['image_crop']);
				...
				//echo "$dst";
				//echo "$filename";die;
				//move_uploaded_file($resize_image, $filename);
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->get['advertise_id'])) {
           ...
           // } else {
            ...
            //$data['cancel'] = $this->url->link('selleradvertise/advertise', ''.$url, 'SSL');
...
//print_r($advertise_info);
            ...
            /*if (isset($this->request->post['offer_image']) && is_file(DIR_IMAGE.$this->request->post['offer_image'])) {
                $data['thumb'] = $this->model_tool_image->resize($this->request->post['offer_image'], 100, 100);
            } elseif (!empty($advertise_info) && is_file(DIR_IMAGE.$advertise_info['offer_image'])) {
                $data['thumb'] = $this->model_tool_image->resize($advertise_info['offer_image'], 100, 100);
            } else {
			...
			/*if (isset($this->request->post['offer_image_original']) && is_file(DIR_IMAGE.$this->request->post['offer_image_original'])) {
                $data['offer_image_original'] = $this->model_tool_image->resize($this->request->post['offer_image_original'], 900, 1200);
            } elseif (!empty($advertise_info) && is_file(DIR_IMAGE.$advertise_info['offer_image_original'])) {
                $data['offer_image_original'] = $this->model_tool_image->resize($advertise_info['offer_image_original'], 900, 1200);
            } else {
                $data['offer_image_original'] = $this->model_tool_image->resize('no_image.png', 100, 100);
            }*/
            ...
            //$this->load->model('selleradvertise/layout');
    ...
    protected function validateForm()
    {
        ...
        //foreach ($this->request->post['product_description'] as $language_id => $value) {
			...
			//if (utf8_strlen($this->request->post['position']) == '') {
...
foreach ($html->find('ul.biblio li span[property="dc:available"]') as $element) {
	...
	public function advertise_mail_to_store_favourites() {
		...
		//echo $this->config->get('sms_sender_ads'); die;
...
//echo "<pre>"; print_r($advertisement_seller_id); die;
				...
				//echo "<pre>"; print_r($cus_email);  die;
