		...
		<div class="row">
			<div class="hovereffect">
				...
				<div class="overlay adv--ress">
					...
					<a class="info" data-toggle="modal" data-target="#myModalad_paid_<?php echo $advetise['advertise_id']; ?>" ><i class="fa fa-arrows-alt"></i></a>
					<a class="info" id="ads_delete_paid<?php echo $advetise['advertise_id']; ?>"><i class="fa fa-trash-o"></i></a>
				</div>
			</div>
		</div>
	</div>
...
<?php if (!empty($advetises)) { ?>
<?php foreach ($advetises as $advetise) { ?>
<div class="modal fade" id="myModalad_paid_<?php echo $advetise['advertise_id']; ?>" role="dialog">
...
<?php foreach ($advetises as $advetise) { ?>
<script>
	$('#ads_delete_paid<?php echo $advetise['advertise_id']; ?>').on('click', function() {
	...
	if(adss)
		 {
		 $.ajax({
			url: 'index.php?route=sellerprofile/sellerprofile/trash&func=paid&advertise_id='+adss,
			type: 'post',
			dataType: 'json',
			success: function(json) {
				if (json['success']) {
					...
					$('#sellerpaidcount').html(json['count']);
