...
$(document).ready(function() {
	...
	$('#grid-view').click(function() {
		...
		if (cols == 2) {
			...
			//$('#content .product-list').attr('class', 'product-layout product-grid col-md-3 col-sm-6 col-xs-12');
		}
...
var favourite = {
	'remove': function(key) {
		$.ajax({
			url: 'index.php?route=common/header/remove',
			type: 'post',
			data: 'key=' + key,
			dataType: 'json',
			beforeSend: function() {
				//$('#favouries_str > button').button('loading');
			},
			complete: function() {
				//$('#favouries_str > button').button('reset');
			},
			success: function(json) {
				// Need to set timeout otherwise it wont update the total
				setTimeout(function () {
