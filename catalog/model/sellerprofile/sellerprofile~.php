...
class Modelsellerprofilesellerprofile extends Model
{
	...
	if (isset($data['nickname'])) {
		if($seller_details['nickname'] != $data['nickname']){		
		  $this->db->query('UPDATE '.DB_PREFIX."customer SET nickname = '".$data['nickname']."', seller_approved = '0', seller_verified = '0' WHERE customer_id='".(int) $this->customer->getId()."'");
		}
	}
	if (isset($data['banner'])) {
		if($seller_details['banner'] != $data['banner']){		
			$this->db->query('UPDATE '.DB_PREFIX."customer SET banner = '".$data['banner']."', seller_approved = '0', seller_verified = '0' WHERE customer_id='".(int) $this->customer->getId()."'");
		}
	}
	...
	if (isset($data['tin'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET tin = '".$data['tin']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
	if (isset($data['pan'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET pan = '".$data['pan']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
	if (isset($data['lat'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET lat = '".$data['lat']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
	if (isset($data['lng'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET lng = '".$data['lng']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
	if (isset($data['owner_name'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET owner_name = '".$data['owner_name']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
	if (isset($data['owner_ll_num'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET owner_ll_num = '".$data['owner_ll_num']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }	
	if (isset($data['store_email'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET store_email = '".$data['store_email']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
	if($seller_details['referred_by'] == '') {
	if (isset($data['referred_by'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET referred_by = '".$data['referred_by']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    } else {
	...
	if (!empty($data['store_mobile_num'])) {
		...
		$this->db->query('UPDATE '.DB_PREFIX."customer SET store_mobile_num = '".$mob."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
	if ((!empty($data['store_ll_code'])) && (!empty($data['store_ll_num']))) {
		...
		$land = implode(",", array_filter($land_line_num));
		$this->db->query('UPDATE '.DB_PREFIX."customer SET store_ll_num = '".$land."' WHERE customer_id='".(int) $this->customer->getId()."'");
	}
	if (isset($data['delivery_type'])) {
      $this->db->query('UPDATE '.DB_PREFIX."customer SET delivery_type = '".$data['delivery_type']."' WHERE customer_id='".(int) $this->customer->getId()."'");
    }
