...
class ModelAccountCustomer extends Model {
	...
	public function editCustomer($data) {
		...
		$telephone_no = $this->customer->getTelephone();

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $telephone_no . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
	...
	public function editCodePhone($phone, $code) {
		$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET code = '" . $this->db->escape($code) . "' WHERE telephone = '" . $phone . "'");
	}
	...
	public function getCustomerByAddress($address_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "address WHERE address_id = '" . $address_id . "'");

		return $query->row;
	}

	public function getCustomerByAddressSecurity($q_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "security_question WHERE q_id = '" . $q_id . "'");
	...
	public function getTotalCustomersByTelephoneOTP($telephone) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE telephone = '" . $this->db->escape($telephone) . "' AND status != '0'");

		return $query->row['total'];
	}

	public function getTotalCustomersByTelephoneCheck($telephone) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE telephone = '" . $this->db->escape($telephone) . "' AND status = '0'");
